...
def add_playlist_item(youtube, playlist_id, video_id):
  ...
  while response is None:
    ...
    try:
      ...
      print "Video id '%s' was added to playlist '%s'." % ( video_id, playlist_id )
    except HttpError, e:
      if e.resp.status in RETRIABLE_STATUS_CODES:
        error = "A retriable HTTP error %d occurred:\n%s" % (e.resp.status,
                                                             e.content)
...
#  add_playlist_item(youtube, "PL1cNOU1HfaMI8ha3e3iUsocQuoXH70CvF", "09XfuAmyaAc")
  ...
  argparser.add_argument("--location", help="Location description",
    default="")
  argparser.add_argument("--latitude", help="Location latitude", default=""),
